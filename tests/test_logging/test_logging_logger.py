...
def test_log_level_value_logging():
    assert all([LogLevel.DEBUG.value == logging.DEBUG,
...
def test_logger_init_set():
    ...
    assert all([logger._log_format == "[%(asctime)s] %(levelname)s: %(message)s",
...
def test_setup_logging():
    ...
    assert all([test_logger.level == LogLevel.DEBUG.value,
...
def test_setup_file_handler():
    test_logger = Logger(file_max_bytes=10, file_backup_count=20, file_level=LogLevel.WARNING, log_format="%(levelname)s")
    assert not hasattr(test_logger, "_file_handler")

    test_logger._setup_file_handler()

    assert all([test_logger._file_handler.maxBytes == 10,
...
def test_track_and_untrack_variable(test_logger: Logger):
    ...
    assert all([test_func in list(test_logger._tracked_vars.values()),
        "Testing" in list(test_logger._var_names.values())])

    test_logger.untrack_variable(test_func)
    assert all([not test_func in list(test_logger._tracked_vars.values()),
        not "Testing" in list(test_logger._var_names.values())])
...
def test_set_file_level(test_logger: Logger):
    ...
    assert all([test_logger._file_level == LogLevel.CRITICAL,
        not hasattr(test_logger, "_file_handler")])

def test_set_file_level_has_attr(test_logger: Logger):
    test_logger._setup_file_handler()
    assert all([hasattr(test_logger, "_file_handler"),
        test_logger._file_handler.mode == "a"])
    
    test_logger.set_file_level(LogLevel.DEBUG)
    assert all([test_logger._file_level == LogLevel.DEBUG,
...
def test_set_stream_level(test_logger: Logger):
    ...
    assert all([hasattr(test_logger, "_stream_handler"),
...
def test_set_format(test_logger: Logger):
    ...
    assert all([test_logger._log_format == "[%(levelname)s]",
...
def test_set_buffer_size(test_logger: Logger):
    ...
    assert all([test_logger._buffer_size == 5,
...
def test_update(test_logger: Logger):    
    ...
    assert all([test_logger._buffer[0] == ['18'],
...
def test_generate_file_paths_no_input_filename(test_logger: Logger):
    ...
    assert all([".log" in test_logger._file_path,
        ".csv" in test_logger._csv_path])
...
def test_reset(test_logger: Logger):
    ...
    test_logger.update()
    test_logger._setup_file_handler()
    assert all([len(test_logger._buffer) == 1,
        len(test_logger._tracked_vars) == 1,
        len(test_logger._var_names) == 1,
        hasattr(test_logger, "_file_handler")])

    test_logger.reset()
    assert all([len(test_logger._buffer) == 0,
