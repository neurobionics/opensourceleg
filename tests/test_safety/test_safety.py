...
def test_is_within_range_max_less_min():
    ...
    min = 4
    max = 1
    with pytest.raises(ValueError, match=f"Maximum value must be greater than minimum value of range"):
        @is_within_range(min,max)
...
def test_add_safety_att_not_prop(mock_print):
    ...
    attribute = "notatt" 
    test_manager.add_safety(samp, attribute, SafetyDecorators.is_positive())
    mock_print.assert_called_once_with(f"Warning: The attribute '{attribute}' is not a property. The SafetyManager only works on properties.")
...
def test_add_safety():
    ...
    assert test_manager._safe_objects[samp] == {attribute: [decorator, decorator2], attribute2: [decorator]}
