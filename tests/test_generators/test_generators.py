...
@pytest.fixture
def step_generator():
    return StepGenerator(step_time=0.0, step_value=2.5, amplitude=1.0, offset=0.5)
...
def composite_generator():
    return CompositeGenerator(
        generators=[SineGenerator(frequency=1.0), StepGenerator(step_time=0.0, step_value=0.5)], operation="add"
    )
...
class TestSignalGeneratorBase:
    ...
    def test_duration_limit(self):
        ...
        assert gen.update(0.1) is not None
        assert gen.update(0.5) is not None
        assert gen.update(0.6) is None  # t=0.6 > duration=0.5
...
class TestStepGenerator:
    def test_constant_value(self, step_generator):
        assert step_generator.update(0.1) == pytest.approx(3.0, abs=TOLERANCE)
        assert step_generator.update(1.0) == pytest.approx(3.0, abs=TOLERANCE)

    def test_step_value(self):
        gen = StepGenerator(step_time=0.5, step_value=1.0)
        assert gen.update(0.4) == pytest.approx(0.0, abs=TOLERANCE)
        assert gen.update(0.5) == pytest.approx(1.0, abs=TOLERANCE)
        assert gen.update(1.0) == pytest.approx(1.0, abs=TOLERANCE)

    def test_noise(self):
        gen = StepGenerator(step_time=0.0, step_value=2.0, noise_amplitude=0.1, seed=SEED)
        values = [gen.update(0.1) for _ in range(3)]
...
class TestSineGenerator:
    def test_key_points(self, sine_generator):
        assert sine_generator.update(0.0) == pytest.approx(3.0, abs=TOLERANCE)
        assert sine_generator.update(0.25) == pytest.approx(1.0, abs=TOLERANCE)
        assert sine_generator.update(0.75) == pytest.approx(1.0, abs=TOLERANCE)

    def test_phase(self):
        gen = SineGenerator(frequency=1.0, phase=math.pi)
        assert gen.update(0.0) == pytest.approx(0.0, abs=TOLERANCE)


class TestRampGenerator:
    def test_slope(self, ramp_generator):
        assert ramp_generator.update(0.0) == pytest.approx(0.0, abs=TOLERANCE)
        assert ramp_generator.update(1.0) == pytest.approx(3.0, abs=TOLERANCE)

    def test_reset_at_duration(self):
        gen = RampGenerator(slope=1.0, reset_time=3.0)
        assert gen.update(1.0) == pytest.approx(1.0, abs=TOLERANCE)
        assert gen.update(2.0) == pytest.approx(2.0, abs=TOLERANCE)
        assert gen.update(3.0) == pytest.approx(0.0, abs=TOLERANCE)  # Reset at 3.0


class TestSquareGenerator:
    def test_duty_cycle(self):
        gen = SquareGenerator(frequency=1.0, duty_cycle=0.3, amplitude=5.0, offset=1.0)
        assert gen.update(0.0) == pytest.approx(6.0, abs=TOLERANCE)
        assert gen.update(0.2) == pytest.approx(6.0, abs=TOLERANCE)
        assert gen.update(0.50) == pytest.approx(-4.0, abs=TOLERANCE)


class TestSawtoothGenerator:
    def test_waveform(self):
        gen = SawtoothGenerator(frequency=1.0, amplitude=2.0, offset=1.0)
        assert gen.update(0.0) == pytest.approx(-1.0, abs=TOLERANCE)
        assert gen.update(0.25) == pytest.approx(0.0, abs=TOLERANCE)
        assert gen.update(0.50) == pytest.approx(1.0, abs=TOLERANCE)
        assert gen.update(1.0) == pytest.approx(-1.0, abs=TOLERANCE)
...
class TestTriangleGenerator:
    def test_symmetry(self):
        ...
        assert gen.update(0.0) == pytest.approx(-1.0, abs=TOLERANCE)
        assert gen.update(0.50) == pytest.approx(3.0, abs=TOLERANCE)
        assert gen.update(0.75) == pytest.approx(1.0, abs=TOLERANCE)
        assert gen.update(1.0) == pytest.approx(-1.0, abs=TOLERANCE)
...
class TestExponentialGenerator:
    def test_decay(self):
        ...
        assert gen.update(0.0) == pytest.approx(0.0, abs=TOLERANCE)
        assert gen.update(1.0) == pytest.approx(2.0 * (1 - 1 / math.e), abs=0.01)

    def test_growth(self):
        gen = ExponentialGenerator(time_constant=1.0, growth=True, amplitude=3.0)
        assert gen.update(0.0) == pytest.approx(3.0, abs=TOLERANCE)
        assert gen.update(1.0) == pytest.approx(3.0 / math.e, abs=0.01)


class TestNoiseGenerators:
    def test_uniform_noise(self):
        gen = UniformNoiseGenerator(amplitude=1.0, seed=SEED)
        values = [gen.update(0.0) for _ in range(3)]
    ...
    def test_reproducibility(self):
        ...
        assert gen1.update(0.1) == gen2.update(0.1)


class TestDataReplayGenerator:
    def test_sequence_replay(self):
        gen = DataReplayGenerator(data=[1, 2, 3], sample_rate=1.0)
        assert gen.update(0.0) == 1
        assert gen.update(1.0) == 2
        assert gen.update(2.0) == 3
        assert gen.update(3.0) == 1  # Looped

    def test_no_loop(self):
        gen = DataReplayGenerator(data=[10, 20], sample_rate=1.0, loop=False)
        assert gen.update(0.0) == 10
        assert gen.update(1.0) == 20
        assert gen.update(2.0) == 20  # Last value remains
...
class TestCustomGenerator:
    def test_simple_expression(self, custom_generator):
        assert custom_generator.update(0.0) == pytest.approx(0.1, abs=TOLERANCE)
        assert custom_generator.update(0.5) == pytest.approx(2.0 * math.sin(math.pi * 0.5) + 0.1, abs=0.1)


class TestCompositeGenerator:
    def test_add_operation(self, composite_generator):
        assert composite_generator.update(0.0) == pytest.approx(0.5, abs=TOLERANCE)
        assert composite_generator.update(0.25) == pytest.approx(1.5, abs=TOLERANCE)

    def test_multiply_operation(self):
        comp_gen = CompositeGenerator(generators=[SineGenerator(), StepGenerator()], operation="multiply")
        assert comp_gen.update(0.0) == pytest.approx(0.0, abs=TOLERANCE)
        assert comp_gen.update(0.25) == pytest.approx(1.0, abs=TOLERANCE)
...
class TestAllGenerators:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (CompositeGenerator, {"generators": [SineGenerator(), StepGenerator()]}),
        ],
    )
...
class TestEdgeCases:
    ...
    def test_large_noise_amplitude(self):
        gen = StepGenerator(step_value=1.0, noise_amplitude=10.0)
        assert -9.0 <= gen.update(0.1) <= 11.0
