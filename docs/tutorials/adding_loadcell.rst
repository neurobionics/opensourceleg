...
This will add a load cell to the ``osl`` object with the specified ``loadcell_matrix``, which is the calibration matrix for the load cell. This calibration matrix is unique to each load cell and can be found in the load cell's datasheet.
