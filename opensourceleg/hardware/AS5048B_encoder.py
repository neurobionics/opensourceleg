...
if __name__ == "__main__":
    ...
    ankle_enc.zero_position = ankle_enc.encoder_output # sets the current position to 0 rad
        # dial_enc.zero_position = dial_enc.encoder_output
    print(f"Zero registers:", knee_enc.zero_position)
    print(f"Enc output:", ankle_enc.encoder_output)
    ...
    while True:
        try:
            ...
            #print("Dial", np.rad2deg(dial_enc.position), np.rad2deg(dial_enc.abs_ang))
            #print(knee_enc.encoder_output)
            # print(ankle_enc.encoder_output)
